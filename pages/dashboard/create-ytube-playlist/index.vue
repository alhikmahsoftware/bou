<template>
    <div class="p-4 sm:ml-64 mt-20">
        <div
            class="mb-5 w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <select @change="change_filter_video_type" id="video_type" v-model="filter_video_type"
                ...
                <option value="" disabled selected hidden>Filter by Video Type</option>
                <option value="formal">Formal</option>
                <option value="non_formal">Non Formal</option>
            </select>
            <select v-if="filter_video_type == 'formal'" @change="fetchFormalFilterDataDepartment" id="school" v-model="filter_School_formal" class="justify-left bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block  p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                <option value="" disabled selected hidden>Select school</option>
                <option :value="item.id" v-for="item in schools" :key="item.id">
                    {{ item.School_name }}
                </option>
            </select>
            <select @change="formalFilter" v-if="filter_departments && filter_departments.length > 0" id="department" v-model="filter_Department_formal"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                <option value="" disabled selected hidden>Select department</option>
                <option :value="item.id" v-for="item in filter_departments" :key="item.id">
            ...
            <div v-else>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr class="border-b dark:border-gray-700" v-for="item in getYoutubePlaylistData" :key="item.id">
                                <th scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ item.id }}</th>
                                <td class="px-4 py-3"> {{ item.title }}</td>
                                <td class="px-4 py-3 max-w-[18rem]">
                                    <a :href="item.url"><img :src="item.thumbnail_url" alt="Image Description"></a>
                                    
                                </td>
                                <td class="px-4 py-3 max-w-[12rem] truncate">
                                    {{
                                        data.find(item2 => item2.id === item.id)
                                        ? data.find(item2 => item2.id === item.id).name
                                        : ''
                                    }}
                                </td>
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div
                                        ...
                                        <ul class="py-1 text-sm">
                                            <li>
                                                <nuxt-link :to="`/dashboard/create-ytube-playlist/${item.id}`"
                ...
                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <form action="#" enctype="multipart/form-data">
                        <div class="grid gap-4 mb-4 sm:grid-cols-2">
                            <div>
                                <label for="category"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Video Type</label>
                                <select id="video_type" v-model="video_type"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    <option value="" disabled selected hidden>Select Video Type</option>
                                    <option value="formal">Formal</option>
                                    <option value="non_formal">Non Formal</option>
                                </select>
                            </div>
                            <div v-if="video_type == 'non_formal'">
                                <label for="category"
                            ...
                            <div v-if="video_type == 'formal'">
                                <label for="school"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">School</label>
                                <select @change="fetchFormalDataDepartment" id="school" v-model="form.School_formal"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    <option value="" disabled selected hidden>Select school</option>
                                    <option :value="item.id" v-for="item in schools" :key="item.id">
                                        {{ item.School_name }}
                            ...
                            <div v-if="video_type == 'formal' && departments && departments.length>0">
                                <label for="department"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Department</label>
                                <select id="department" v-model="form.Department_formal"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    <option value="" disabled selected hidden>Select department</option>
                                    <option :value="item.id" v-for="item in departments" :key="item.id">
                                        {{ item.Department_name }}
                                    </option>
                                </select>
                            </div>
                             ...
                             <div class="sm:col-span-2">
                                ...
                                <input type="text" name="url" id="url"
                                    ...
                                    placeholder="Type youtube url" required="" v-model="form.url" />
                            </div>
                        </div>
...
import { storeYoutubePlaylist } from "../../data/dashboard/youtube-play-list-api/store-youtube-playlist";
import { deleteYoutubePlayList } from "../../data/dashboard/youtube-play-list-api/delete-youtube-playlist";
...
const schools = ref([]);
const getYoutubePlaylistData = ref([]);
const loading = ref(true);
const video_type = ref('');
const filter_video_type = ref('');
const filter_School_formal = ref('');
const filter_Department_formal = ref('');
...
const confirmDelete = async () => {
    ...
    if (itemId) {
        try {
            await deleteYoutubePlayList(itemId);
...
//get category
async function change_filter_video_type() {
    if(filter_video_type.value == 'non_formal'){
        fetchData()
        filter_departments.value=[];
        filter_School_formal.value='';
        filter_Department_formal.value='';
    }
}
async function fetchData() {
    try {
        ...
        getYoutubePlaylistData.value = await getYoutubePlaylist();
        console.log(data);
    } catch (error) {
...
async function formalFilter() {
    try {
        loading.value = true;
        getYoutubePlaylistData.value = await formalVideo(filter_Department_formal.value,filter_School_formal.value);
        console.log(data);
    } catch (error) {
        console.error("Error fetching data:", error);
    } finally {
...
//get formal category
async function fetchFormalData() {
    try {
        loading.value = true;
...
async function fetchFormalDataDepartment() {
    if(form.School_formal ){
        try {
            loading.value = true;
            departments.value = await getDepartmentsCategory(form.School_formal);
        } catch (error) {
            console.error("Error fetching data:", error);
        } finally {
            loading.value = false;
...
async function fetchFormalFilterDataDepartment() {
    if( filter_School_formal.value){
        try {
            loading.value = true;
            filter_departments.value = await getDepartmentsCategory(filter_School_formal.value);
        } catch (error) {
            console.error("Error fetching data:", error);
        } finally {
            loading.value = false;
...
const handleSubmit = async (event) => {
    ...
    try {
        const result = await storeYoutubePlaylist(video_type.value,form);
