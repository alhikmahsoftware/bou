            ...
            <div v-else>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr class="border-b dark:border-gray-700" v-for="(item, index) in getSchoolData"
                                :key="index">
                                <td scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ index + 1 }}</td>
                                <td class="px-4 py-3"> {{ item.School_name }}</td>
                                <td class="px-4 py-3">
                                    <div
                                        class="z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm">
                                            <li>
                                                <nuxt-link :to="`/dashboard/create-routine-school/${item.id}`"
                        ...
                        <div class="grid gap-4 mb-4 sm:grid-cols-2">
                            <div class="">
                                ...
                                <input type="text" name="title" id="title"
                                    ...
                                    placeholder="Type title" required="" v-model="form.School_name" />
                            </div>
                        </div>
                        <button type="submit"
...
import { storeSchool } from "../../data/dashboard/class-routine-api/store-school";
import { deleteRoutineSchool } from "../../data/dashboard/class-routine-api/delete-routine-school";
...
const confirmDelete = async () => {
    ...
    if (itemId) {
        try {
            await deleteRoutineSchool(itemId);
...
const handleSubmit = async () => {
    ...
    try {
        const result = await storeSchool(form);
